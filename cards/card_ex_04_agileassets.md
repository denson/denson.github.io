...
order: 2
bullets:
  - "Built a SageMaker-based computer vision system to detect highway guardrails in vehicle-captured imagery and classify damaged vs OK cases for human review."
  - "Designed an anomaly detection framing to handle extreme class imbalance (damaged cases <1% of training data) and long-tail guardrail variants."
  - "Implemented an iterative error-driven labeling loop with SageMaker Ground Truth, capturing every model mistake to prioritize high-value labels and rapid retraining."
  - "Delivered a prototype in ~2 weeks and production-ready system in ~1 month, achieving high recall on true damaged guardrails while keeping review workload acceptable."
aiContextMarkdown: |-
  **Situation**
  - State DOTs needed efficient detection of damaged highway guardrails; manual inspection by driving roads was time-intensive, expensive, and inconsistent.

  **Constraints**
  - Severely imbalanced data (damaged cases <1% of training data) and long-tail guardrail variants.
  - Oracle-to-AWS data egress costs and licensing constraints affecting Linear Referencing System (LRS) precision.

  **Action**
  - Built a YOLO-based computer vision pipeline on AWS SageMaker with iterative, error-driven labeling.
  - Framed damaged-vs-OK as anomaly detection to handle class imbalance and long-tail normal variants.
  - Implemented retraining loops that capture mistakes and learn context-specific normal configurations.

  **Result**
  - Prototype in ~2 weeks, production-ready in ~1 month.
  - Very high recall on truly damaged guardrails while keeping false positives manageable for human review.
  - Enabled broader, more consistent coverage than manual spot-checks.

  **Lesson learned**
  - The biggest lever wasn’t “label everything”—it was using the model to surface what it doesn’t understand, then labeling those cases and retraining. This pattern maps directly to LLM systems: route uncertain outputs to humans, harvest corrections, and continuously improve.
---
...
Action: Built a YOLO-based computer vision pipeline on AWS SageMaker with iterative error-driven labeling; framed damaged-vs-OK as anomaly detection to handle class imbalance; implemented retraining loops that capture mistakes and learn context-specific normal configurations.

Result: Prototype in ~2 weeks, production-ready in ~1 month. Achieved very high recall on truly damaged guardrails while keeping false positives manageable for human review; enabled broader, more consistent coverage than manual spot-checks.

Lesson learned: The biggest lever wasn't “label everything”—it was using the model to surface what it doesn’t understand, then labeling those cases and retraining. This pattern maps directly to LLM systems: route uncertain outputs to humans, harvest corrections, and continuously improve.

---

### Why it mattered (problem framing)

State DOTs hire contractors to repair and maintain guardrails. Traditionally:

* A state employee must **drive roads to find damaged guardrails**.
* Then **drive back to verify repairs**.

This is time-intensive, expensive, and inconsistent (coverage varies, conditions vary, and rare cases are easy to miss).
